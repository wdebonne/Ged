...
export default function ImportModal({ isOpen, onClose }) {
  ...
  // Nettoyage de l'URL du blob quand le modal se ferme ou le composant est démonté
  useEffect(() => {
    return () => {
      if (pdfUrl) {
  ...
  // Callback pour react-pdf quand le document est chargé
  ...
  const createMailMutation = useMutation({
    ...
    onError: (error) => {
      ...
      const message = error.response?.data?.message || 'Erreur lors de la création du courrier';
  ...
  const handleSubmit = (e) => {
    ...
    // S'assurer que senderName est défini si pas de senderId
    const dataToSubmit = {
      ...formData,
      senderName: formData.senderName || senderSearch
    };
    
    createMailMutation.mutate(dataToSubmit);
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {!file ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-primary-50 rounded-lg border border-primary-200 dark:bg-primary-900/20 dark:border-primary-900/40">
                    ...
                    {/* Aperçu PDF avec react-pdf */}
                    <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 flex flex-col items-center overflow-hidden">
                      {pdfUrl && (
                        <>
                          <Document
                            file={pdfUrl}
                          ...
                          {numPages && (
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                              {numPages} page{numPages > 1 ? 's' : ''} au total
                            </p>
                          )}
                        </>
                      )}
